...
struct GeometricMiniWebViewBox: UIViewControllerRepresentable {
    // MARK: - Свойства

    @ObservedObject var loader: GeometricMiniWebLoader
    var defaultOrientations: UIInterfaceOrientationMask = .portrait

    // MARK: - Координатор

    func makeCoordinator() -> GeometricMiniWebCoordinator {
        GeometricMiniWebCoordinator { [weak loader] status in
    ...
    func makeUIViewController(context: Context) -> GeometricMiniBaseWebViewController {
        let configuration = createWebViewConfiguration()
        return GeometricMiniBaseWebViewController(
    ...
    func updateUIViewController(_ uiViewController: GeometricMiniBaseWebViewController, context: Context) {
