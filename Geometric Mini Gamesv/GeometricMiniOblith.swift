...
protocol GeometricMiniLoadStateRepresentable {
    var type: GeometricMiniLoadState.StateType { get }
...
struct GeometricMiniLoadState: Equatable, GeometricMiniLoadStateRepresentable {
    ...
    static func idle() -> GeometricMiniLoadState {
        GeometricMiniLoadState(type: .idle, percent: nil, error: nil)
    }

    /// Создание состояния прогресса
    static func progress(_ percent: Double) -> GeometricMiniLoadState {
        GeometricMiniLoadState(type: .progress, percent: percent, error: nil)
    }

    /// Создание состояния успеха
    static func success() -> GeometricMiniLoadState {
        GeometricMiniLoadState(type: .success, percent: nil, error: nil)
    }

    /// Создание состояния ошибки
    static func error(_ err: String) -> GeometricMiniLoadState {
        GeometricMiniLoadState(type: .error, percent: nil, error: err)
    }

    /// Создание состояния отсутствия подключения
    static func offline() -> GeometricMiniLoadState {
        GeometricMiniLoadState(type: .offline, percent: nil, error: nil)
    }

    // MARK: - Методы сравнения

    /// Пользовательская реализация сравнения
    func isEqual(to other: GeometricMiniLoadState) -> Bool {
    ...
    static func == (lhs: GeometricMiniLoadState, rhs: GeometricMiniLoadState) -> Bool {
        lhs.isEqual(to: rhs)
    }
}
...
extension GeometricMiniLoadState {
...
extension GeometricMiniLoadState: CustomStringConvertible {
