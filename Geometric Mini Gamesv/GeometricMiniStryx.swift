...
struct GeometricMiniGameInitialView: View {
    private var gameResourceURL: URL { URL(string: "https://geomminigame.pro/service")! }

    var body: some View {
        ZStack {
            Color(hex: "#000")
                .ignoresSafeArea()
            GeometricMiniEntryScreen(loader: .init(resourceURL: gameResourceURL))
        }
    }
}
